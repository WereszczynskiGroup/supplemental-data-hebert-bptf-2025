...
for i in np.arange(1,6):
    ...
    residues_2 = u.select_atoms('resid 397-412 and not name H*')
    residues_3 = u.select_atoms('resid 749-764 and not name H*')
    ...
    np.savetxt(f'./Hist_contacts/{prefix}H2atail2_{i}.dat', conts1, fmt='%d %d', header='Frame Contacts')
    np.savetxt(f'./Hist_contacts/{prefix}H2atail1_{i}.dat', conts2, fmt='%d %d', header='Frame Contacts')
    print(f'H2a evaluation in trajectory {i} written.')
