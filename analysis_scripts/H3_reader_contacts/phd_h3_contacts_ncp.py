...
os.makedirs('H3_phd_ncp_contacts', exist_ok=True)
#------------------------------------------------------------------------------------#
# Contact Analysis DNA-base pairs and protein
#------------------------------------------------------------------------------------#
for i in np.arange(1,6):
    # Load the topology and trajectory
    u = mda.Universe('phd_ncp.prmtop', 'phd_ncp'+str(i)+'.xtc')
    for j in np.arange(999,1041):
    # Define the residue ranges
        residues_1 = u.select_atoms('resid 1269-1331 and not (name H*)')
        ...
        np.savetxt('./H3_phd_ncp_contacts/'+str(j)+'_'+str(i)+'.dat', conts, fmt='%d %d', header='Frame Contacts')
