...
os.makedirs('bptf_peptide_h3_contacts', exist_ok=True)
#------------------------------------------------------------------------------------#
...
for i in np.arange(1,6):
    ...
    u = mda.Universe('bptf_peptide.prmtop', 'bptf_peptide'+str(i)+'.xtc')
    residues_1 = u.select_atoms('resid 1-170 and not (name H*)')
    for j in np.arange(171,186):
    # Define the residue ranges
        ...
        np.savetxt('./bptf_peptide_h3_contacts/'+str(j)+'_'+str(i)+'.dat', conts, fmt='%d %d', header='Frame Contacts')
